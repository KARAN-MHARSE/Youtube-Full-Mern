...
function comment({ likeSendData }) {
  ...
  useEffect(() => {
    const start = async () => {
      const res = await fetch(
        `https://youtube-full-mern-1.onrender.com/api/v1/user/video/getVideoByID/getComment/${likeSendData.videoId}`
      );
  ...
  const doComment = async (e) => {
    ...
    const res = await fetch(
      `https://youtube-full-mern-1.onrender.com/api/v1/user/video/getVideoByID/doComment/${likeSendData.videoId}`,
      {
        method: "POST",
        headers: {
  ...
  return (
    <div className="w-full  cursor-pointer p-3 my-4  bg-cardBg rounded-lg ">
      ...
      <div className="w-full mb-3">
        <div className="flex items-center gap-3  ">
          ...
          <div className="bg-blue-500 size-[35px] rounded-full flex items-center justify-center font-bold">
            <p>
              {currentUser ? currentUser.userName.charAt(0).toUpperCase() : "X"}
            </p>
          </div>
          {currentUser ? (
